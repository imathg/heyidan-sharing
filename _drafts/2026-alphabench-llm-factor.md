...
LLM 能生成因子公式，不等于能仅凭公式文本判断回测绩效。[AlphaBench](#ref-alphabench) 将公式化 alpha 因子挖掘分成生成、评估和搜索三部分，使用 CSI300 数据与 [Qlib](#ref-qlib) 回测；本文重点看其中的绩效判断：零样本预测是否可靠，成对选择微调带来什么改善。
...
[公开解读](#ref-alphabench-explainer) 列出的零样本结果包括：DeepSeek-V3 的信号 / 噪声分类准确率为 0.46，成对选择为 0.48；GPT-5 的成对选择在 CSI300 为 0.64，在 SP500 为 0.52。这些配置的结果跨市场差异明显，不能据此承诺稳定的绩效判断，也不足以断言所有 LLM 都没有判别能力。
...
`[本文归纳]` LLM 适合充当变异 / 杂交算子的候选生成器，回测承担最终约束。这是当前较稳妥的 LLM 量化因子挖掘路径：LLM 负责生成候选多样性，成对微调模型可以辅助筛选，绩效判断仍需回测验证。
