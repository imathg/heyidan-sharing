# 全双工语音 Agent 评测：输入、播放与打断状态如何对齐
<!-- domain: realtime-interactive-agent -->
<!-- edition_date: 2026-08-23 -->

`[本文归纳]` 系统已经生成两个选项，但只播放完第一个，用户就在中途插话。下一轮模型该根据完整生成文本理解用户，还是只根据用户可能听到的部分？类似地，转写文本完整不代表模型取得了语气线索，任务最终成功也不代表打断与恢复过程正确。

`[本文归纳]` 这个分析样例用来说明三种容易被混记的状态：模型实际取得的输入、客户端实际播放的内容、用户下一轮的反应。它不是某次真实事故；下文分别用已有研究核对这些环节，不把完整错误链当作已验证的因果结论。

`[本文归纳]` 服务端生成进度与客户端播放进度不是同一时间线。只有分开记录输入、生成、播放、打断和后续反应的时刻，才能判断错误来自模型感知、上下文更新、语音合成还是对话策略。

`[本文归纳]` 本文讨论怎样取得这类分层证据，以及哪些反事实对照可以定位问题。它不将“播放过”当作“用户理解了”，也不将局部状态修复当作真实体验已经提升。
...
<a id="7-三本账-分层评测与反事实归因"></a>

## 7. 对齐输入、播放和用户反应，再做单层对照
