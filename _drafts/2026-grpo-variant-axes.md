...
<svg viewBox="0 0 760 250" width="100%" role="img" aria-label="GRPO variant axes map" style="border:1px solid #2a3441;border-radius:4px;background:#0a1018;">
  ...
  <text x="48" y="236" class="small">Dr-GRPO / DAPO / CISPO / BPPO / CARL 是五个设计轴上的不同位点。</text>
</svg>
</figure>

## 先把名字归入五个设计轴

`[本文归纳]` 一个 GRPO-style 变体至少要回答五个问题。无法回答时，它更像某个轴上的实现细节。
...
`[论文]` [DAPO](#ref-yu2025)（ByteDance Seed + AIR Tsinghua + HKU + SIA-Lab，Yu et al. 2025）适合作为入口，因为它在同一篇系统报告的消融实验中检查了多条设计轴：Naive GRPO 在 AIME 2024 上是 30，逐步加入 Overlong Filtering、Clip-Higher、Soft Overlong、Token-level loss、Dynamic Sampling 后到 50。这个表说明，GRPO 更像一套可以逐项替换的 update-signal 管线。
...
`[论文]` 这个设计的价值在于独立性。[BPPO](#ref-zhao2026) 明确保留 full-group sampling、reward 和 group-relative advantage normalization，只改 update 的作用位置。MATH setup 下，BPPO 报告 mean response length 降低 50.6%，训练 wall-clock speedup 达到 6.08x；abstract 将 30-50% 的长度下降归于 prefix-focused optimization。BPPO 的核心设计轴是 A4：**谁承载更新信号**。

`[本文归纳]` 这也是判断独立设计轴的判据。一个轴应当像 BPPO 一样固定其他组件，只移动一个变量，并给出对应可观测量：训练 token 变少、长度变短、速度变快，准确率仍可接近 GRPO。如果一个改法同时改 reward、clip、sample filtering 和 aggregation，它仍可能有效，但更难被视为独立设计轴。

## DAPO / CISPO 解释长 CoT 的 clip 压力

`[论文]` [DAPO](#ref-yu2025) 的 Clip-Higher 直接指向 A2。论文解释 upper clip 会限制 low-probability token 的 probability increase，而长 CoT 里的探索词往往本来就低概率。Clip-Higher 给这些 token 留出更大的上升空间；Dynamic Sampling 则把全对或全错的 prompt 过滤掉，防止 batch 被没有有效梯度的样本占满。

`[论文]` 同一篇 [DAPO](#ref-yu2025) 还将 A5 纳入讨论：sample-level loss 会将长 CoT 的影响不恰当地平均，token-level loss 让长序列按 token 数贡献梯度；Soft Overlong / Overlong Filtering 则通过 reward shaping 或过滤施加长度压力。这些改法属于长推理训练动力学，比表层 reward 调整更接近更新机制。

`[tech report]` [CISPO 发现过程](#ref-cispo) 来自 MiniMax 相关贡献者的复盘，证据等级低于论文，但解释路径明确：off-policy clip=0.2 会抑制 response length 增长，被 clip 的少数 token 里包含 `wait`、`but`、`let me` 等 long-CoT 涌现关键词。CISPO 第一版 loss 对正 advantage 走 detach-clip importance sampling 乘 `log pi`，对负 advantage 走 standard PPO，因为负梯度对 entropy 更友好。

`[本文归纳]` DAPO 和 CISPO 的共同点在于：二者都表明 A2 会改变探索 token 的更新路径。长 CoT 训练里，ratio clipping、advantage normalization、length aggregation 会共同决定低概率 token 获得的增益、截断或长度项稀释。

## agentic RL 把 group 从 prompt 迁到 state / segment

`[tech report]` [Long-horizon GRPO 综述](#ref-longhorizon) 将 agentic GRPO 的关键变化归为「什么样本进入同一个 group 比较」。Vanilla GRPO / Search-R1 / RAGEN-StarPO 仍按同 prompt 的完整 trajectory 分组；GiGPO 换成 episode + anchor-state；HGPO 要求 context-consistent；Tree-GRPO 比较 sibling branches；ARPO / AEPO 将预算集中到高熵 tool-use 点。
...
`[论文]` [BPPO](#ref-zhao2026) 之所以是清晰的单变量案例，是因为它保留 full-group advantage normalization，只将 carrier 换成最短 correct / incorrect prefix。`[tech report]` [Dr-GRPO pin](#ref-drgrpo) 单列是因为它直接作用于 A1：去掉 group std normalization 和 response length normalization。两者结合可解释这个退化角：当 group 的相对比较变弱，baseline、normalization、carrier 不再像五轴表中那样清楚分离。
...
`[本文归纳]` 第二关是**干净性**：效果是否被转嫁。一个改法可以独立，却仍然不干净；它降低目标 proxy 的同时，可能将优化压力转移到相邻 proxy。overlong shaping、length penalty、entropy trigger 这类改法常常落在这里。它们可能有效，但需要同时监控长度、正确率、entropy、tool-use rate 或其他相邻指标，验证优化压力是否仍停留在目标 proxy 上。

`[论文]` [BPPO](#ref-zhao2026) 在这里提供了一个反例参照：不显式加 length penalty，也能让平均长度下降 30-50%。这意味着长度不自动具备独立轴资格。长度可能是 reward pressure 的目标，也可能是 update carrier 的副产物，还可能是 clip/normalization 对探索 token 的间接影响。

`[本文归纳]` 更实用的读法是：看到一个 GRPO 新名词，先填这张表，再判断它的论文定位。如果某个名字只能填进一个格子，它是局部设计轴；如果它跨多个格子，需要看 ablation 是否能把格子拆开；如果它通过独立性但过不了干净性，就把它写成 pressure routing，保留新轴资格的判断。
...
`[本文归纳]` 论文名适合检索，配置名适合落地；二者宜使用同一套语言。工程上遇到训练症状时，先填轴表，再决定读哪篇论文。
...
`[本文归纳]` 按此阅读，Dr-GRPO / DAPO / CISPO / BPPO / CARL / ReMax 可视为同一组设计轴的不同取值。新论文若只改一个轴，可按该轴吸收；若同时改多个轴，应先要求拆分 ablation；若声称开辟新轴，则应给出独立性和干净性证据。

## 可证伪问题

`[本文归纳]` 这篇 brief 的主结论可以被三类实验证伪。第一，如果有主流 GRPO 变体无法映射到 A1-A5 任意一轴，五轴表不完备。第二，如果 BPPO 的收益在 random pair 或 same-class pair 上同样成立，A4 的 correct/incorrect contrast 解释就弱。第三，如果 CARL 的 segment critic 被 token-level PPO 完全追平，agentic 场景里的主因就要从「边界对齐 credit」转向其他机制。

`[本文归纳]` 在这些反例出现前，将 GRPO 变体视为五个 update-signal 设计轴，比按论文名罗列更能直接指导选型：要省 token，先看 A4；要改善长 CoT 探索，先看 A2 和 A1；要做 tool-use agent，先看 A4/A5 的 state 或 segment 边界；要解释方差和 bias，先看 A3。
