...
> `[本文归纳]` OPD 的监督可靠性需要随训练状态检查：长 prefix 可能削弱 teacher 的纠偏信号，师生分布失配可能使优化失败，异步旧 rollout 则可能使损失使用过期的 student 分布。三者需要的检测量不同，不能只凭 KL 下降判断训练有效。

下文用“稳定域”概括监督仍可用的训练条件，它只是本文组织材料的比喻，不是已有统一指标或证明的数学区域。[OPD 机制篇](../on-policy-distillation/) 讨论了高概率 token 交集和伪收敛；本文进一步对照长生成、师生失配、异步训练等失败，并说明相应修复的适用范围。
